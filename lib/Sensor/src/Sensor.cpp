...
StringArray2 Sensor::getCritMessages() {
    ...
    criticalValues.arr[0] = this->shortName + " at " + this->readSensor(); // first message (ex. "Light: 50%")
    ...
    if (status == State::NORMAL) // if value is normal
        ...
        criticalValues.arr[1] = "Incr to "; // second message (ex. "Increase to 50% - 80%")
    criticalValues.arr[1] += String(this->critPercRange.getMinValue()) + "-" + String(this->critPercRange.getMaxValue()) + sign; // add critical percentage range to second message
...
State Sensor::isCritical() {
    ...
    if (map(value, this->minMaxRange.getMinValue(), this->minMaxRange.getMaxValue(), 0, 100) < this->critPercRange.getMinValue()) { // if value is below lowCritPerc
        Serial.println(this->name + " value (" + String(value) + "%) is too low!"); // print value to serial monitor
        ...
        Serial.println(this->name + " value (" + String(value) + "%) is too high!"); // print value to serial monitor
